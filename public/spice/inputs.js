...
/* Virtual cursor for pointer lock.
   _pl_seeded: false until we seed from the VM's last known cursor position,
   ensuring no jump when capture starts. No edge clamping — the SPICE server
   handles out-of-bounds coords, giving unrestricted movement feel. */
var _pl_vx = 0;
var _pl_vy = 0;
var _pl_seeded = false;
...
function handle_mousemove(e)
{
    ...
    {
        /* Seed once from the VM's last known cursor position so the
           in-VM cursor doesn't jump when capture is first acquired. */
        if (!_pl_seeded)
        {
            _pl_vx = (this.sc && this.sc.mousex !== undefined) ? this.sc.mousex : this.width  / 2;
            _pl_vy = (this.sc && this.sc.mousey !== undefined) ? this.sc.mousey : this.height / 2;
            _pl_seeded = true;
        }
        _pl_vx += e.movementX;
        _pl_vy += e.movementY;
        /* No clamping — SPICE server clips at screen bounds */
        var rect = this.getBoundingClientRect();
        e = {
            offsetX:        _pl_vx,
            offsetY:        _pl_vy,
            pageX:          rect.left + _pl_vx,
            pageY:          rect.top  + _pl_vy,
            preventDefault: e.preventDefault.bind(e),
        };
    }
    else
    {
        _pl_seeded = false; /* reset so next lock seeds cleanly */
    }
