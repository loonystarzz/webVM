...
import {
  createVM, getVM, getVMStatus, ensureVMRunning, getVMSpicePort,
  hardReset, hardPowerOff, gracefulShutdown, powerOn
...
// POST /api/vm/:code/hard-reset — resets VM; SPICE reconnects automatically
vmRouter.post('/:code/hard-reset', async (req, res) => {
  const code = req.params.code.toUpperCase();
  const meta = await getVM(code);
  if (!meta) return res.status(404).json({ error: 'VM not found' });

  try {
    await hardReset(meta.vmName);
...
// POST /api/vm/:code/hard-poweroff — kills VM immediately, no auto-reconnect
vmRouter.post('/:code/hard-poweroff', async (req, res) => {
  const code = req.params.code.toUpperCase();
  const meta = await getVM(code);
  if (!meta) return res.status(404).json({ error: 'VM not found' });

  try {
    await hardPowerOff(meta.vmName);
    res.json({ success: true });
  } catch (err) {
    console.error('hard-poweroff error:', err);
    res.status(500).json({ error: err.message });
  }
});

// POST /api/vm/:code/shutdown — ACPI graceful shutdown
vmRouter.post('/:code/shutdown', async (req, res) => {
  const code = req.params.code.toUpperCase();
  const meta = await getVM(code);
  if (!meta) return res.status(404).json({ error: 'VM not found' });

  try {
    await gracefulShutdown(meta.vmName);
    res.json({ success: true });
  } catch (err) {
...
// POST /api/vm/:code/poweron — starts a shut-off VM
vmRouter.post('/:code/poweron', async (req, res) => {
  const code = req.params.code.toUpperCase();
  const meta = await getVM(code);
  if (!meta) return res.status(404).json({ error: 'VM not found' });
