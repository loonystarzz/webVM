...
const EXTRA_DIR = path.join(WEBROOT, 'extra');
const GOODIES_ISO = path.join(EXTRA_DIR, 'goodies.iso');
...
function buildXml({ code, type, imagePath, isoPath, goodiesPath, mac, vmName }) {
...
export async function createVM(type) {
  ...
  // Verify goodies.iso is present
  try {
    await fs.access(GOODIES_ISO);
  } catch {
    throw new Error(`goodies.iso not found at ${GOODIES_ISO} — place it in the ./extra directory`);
  }

  const mac = generateMac();
  const vmName = cfg.libvirtName(code);
  const xml = buildXml({ code, type, imagePath, isoPath, goodiesPath: GOODIES_ISO, mac, vmName });
  const xmlPath = path.join(vmFolder, 'domain.xml');
  await fs.writeFile(xmlPath, xml, 'utf-8');

  // Save metadata
  const now = new Date().toISOString();
  const meta = { code, type, vmName, mac, imagePath, isoPath, goodiesPath: GOODIES_ISO, xmlPath, created: now, lastAccessed: now };
...
export async function getVM(code) {
  ...
  const metaPath = path.join(vmFolder, 'meta.json');
  let meta;
  try {
  ...
  // Update lastAccessed every time the VM is looked up
  meta.lastAccessed = new Date().toISOString();
  try {
    await fs.writeFile(metaPath, JSON.stringify(meta, null, 2));
    console.log('[access] Updated lastAccessed for VM ' + code);
  } catch (err) {
    console.error('[access] Failed to write lastAccessed for VM ' + code + ':', err);
  }
  return meta;
}
...
export async function hardReset(vmName) {
  await virsh('reset', vmName);
}

export async function hardPowerOff(vmName) {
  await virsh('destroy', vmName);
}

export async function gracefulShutdown(vmName) {
  await virsh('shutdown', vmName);
}

export async function powerOn(meta) {
  const state = await getVMStatus(meta.vmName);
  if (state === 'running') return; // already on
  try { await virsh('define', meta.xmlPath); } catch {}
  await virsh('start', meta.vmName);
}

export async function deleteVM(code) {
  const vmFolder = path.join(VM_DIR, code.toUpperCase());
  let meta;
  try {
    ...
    // No meta — just try to remove the folder
    try { await fs.rm(vmFolder, { recursive: true, force: true }); } catch {}
    return;
  }

  // Stop & undefine from libvirt (best-effort)
  try { await virsh('destroy', meta.vmName); } catch {}
  try { await virsh('undefine', meta.vmName); } catch {}

  // Remove VM folder from disk
  await fs.rm(vmFolder, { recursive: true, force: true });
}

export async function touchVMAccess(code) {
  const vmFolder = path.join(VM_DIR, code.toUpperCase());
  const metaPath = path.join(vmFolder, 'meta.json');
  try {
    const raw = await fs.readFile(metaPath, 'utf-8');
    ...
    meta.lastAccessed = new Date().toISOString();
    await fs.writeFile(metaPath, JSON.stringify(meta, null, 2));
    console.log(`[touch] Updated lastAccessed for VM ${code}`);
  } catch (err) {
    console.error(`[touch] Failed to update lastAccessed for VM ${code}:`, err);
  }
}
...
export async function runCleanup() {
  ...
  try {
    ...
    return; // VM_DIR doesn't exist yet
  }

  const now = Date.now();
  for (const entry of entries) {
    const metaPath = path.join(VM_DIR, entry, 'meta.json');
    try {
      const raw = await fs.readFile(metaPath, 'utf-8');
      const meta = JSON.parse(raw);

      const created      = meta.created      ? new Date(meta.created).getTime()      : 0;
      const lastAccessed = meta.lastAccessed  ? new Date(meta.lastAccessed).getTime() : created;

      const inactiveMs   = now - lastAccessed;

      if (inactiveMs >= STALE_MS) {
        console.log(`[cleanup] Deleting inactive VM ${entry} (inactive ${Math.round(inactiveMs/86400000)}d)`);
        await deleteVM(entry);
      }
    } catch {
...
export function startCleanupScheduler(intervalMs = 60 * 60 * 1000) {
  // Run once at startup, then on every interval
  runCleanup().catch(err => console.error('[cleanup] startup error:', err));
  setInterval(() => {
    runCleanup().catch(err => console.error('[cleanup] interval error:', err));
  }, intervalMs);
}
